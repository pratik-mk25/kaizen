...
import crud
from auth import get_current_user, club_pc_only
...
@router.get("/kiosk")
async def attendance_kiosk(request: Request, _=Depends(club_pc_only), user: dict = Depends(get_current_user)):
...
async def kiosk_toggle(request: Request, user_id: str = Form(...), _=Depends(club_pc_only),
...
async def confirm_edit(request: Request, attendance_id: str = Form(...), code: str = Form(...),
                       ...
                       action: str = Form(...), _=Depends(club_pc_only),
                       user: dict = Depends(get_current_user)):
