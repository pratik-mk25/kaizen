...
async def list_users(request: Request, user: dict = Depends(admin_required)):
    ...
    profiles = crud.get_all_users_detailed(None if is_editor else org_id)
...
async def change_user_role(user_id: str, new_role: str = Form(...), user: dict = Depends(admin_required)):
    if new_role not in ("member", "lead", "admin", "editor"):
        return HTMLResponse("Invalid role", status_code=400)
    
    is_editor = user.get("role") == "editor"
    org_id = user.get("organization_id")
    target_profile = next((p for p in crud.get_all_users_detailed(None if is_editor else org_id) if p["id"] == user_id), None)
    ...
    crud.log_action(user["id"], "role_updated", "user", user_id,
                    old_values={"role": "..."}, new_values={"role": new_role}, org_id=target_profile.get("organization_id"))
...
async def add_user_form(request: Request, user: dict = Depends(admin_required)):
    ...
    return render_template("admin_add_user.html", request, user=user, organizations=orgs)
...
async def add_user_action(request: Request,
                          ...
                          role: str = Form(...), 
                          target_org_id: str = Form(None),
                          user: dict = Depends(admin_required)):
    if role not in ("member", "lead", "admin", "editor"):
        return HTMLResponse("Invalid role", status_code=400)
    
    is_editor = user.get("role") == "editor"
    # Editors use target_org_id, admins use their own
    org_id = target_org_id if is_editor else user.get("organization_id")
    
    try:
        crud.create_user_by_admin(email, password, display_name, role, user["id"], org_id)
    except Exception as e:
        orgs = []
        if is_editor:
            orgs = supabase.table("organizations").select("id, name").execute().data or []
        return render_template("admin_add_user.html", request, user=user, error=str(e), organizations=orgs)
    return RedirectResponse(url="/admin/users", status_code=303)
...
async def audit_log_view(request: Request, user: dict = Depends(admin_required)):
    is_editor = user.get("role") == "editor"
    org_id = user.get("organization_id")
    logs = crud.get_audit_logs(limit=100, org_id=None if is_editor else org_id)
